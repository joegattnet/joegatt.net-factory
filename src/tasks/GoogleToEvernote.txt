...
// The file token.json stores is created automatically 
// when authorization completes for the first time.
...
fs.readFile(CREDENTIALS_PATH, (err: string, content: string) => {
...
const formatFootnote = (footnoteId: string, footnotes: string) => {
 ...
 ******************************************************************************/
 ...
 function googleToEvernote(auth: GoogleCredentials) {
  ...
  docs.documents.get({
    ...
    fs.writeFile(filePath, bodyText, (err: string) => {
      if (err) return console.error(err);
      console.log(`Content stored to ${fileName}`);
    });

    updateEvernoteNote(noteStore, text.evernoteId, bodyText, res.data.title);
  });
}
